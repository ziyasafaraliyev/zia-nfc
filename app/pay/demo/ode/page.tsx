...
export default function DemoOdePage() {
  const {
    cartLines,
    cartTotal,
    splitTotal,
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-sm font-black text-slate-950">
              Tam məbləği ödə · {formatAz(cartTotal)}
            </p>
            <p className="mt-1 text-xs text-slate-500">
              Bütün səbət bir nəfər tərəfindən
            </p>
          </div>
          {payMode === "full" && (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="space-y-2">
            {cartLines.map((line) => {
              ...
              const selectedTotal = unitPrice * selected;
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-slate-950">
                      {formatAz(selectedTotal)}
                    </span>
                  </div>

                  {/* Miqdar seçimi — 2+ ədəd olanda mütləq; 1 olanda da eyni UI */}
                  {line.quantity >= 1 && (
