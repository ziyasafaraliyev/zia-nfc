...
import { Globe, Instagram, MapPin, MessageCircle, Music2, Phone, QrCode, UserPlus } from "lucide-react";
      ...
      <section className="mx-auto max-w-md overflow-hidden rounded-[2.2rem] border border-white/10 bg-white text-slate-950 shadow-[0_34px_110px_rgba(0,0,0,0.35)]">
        ...
        >
          <div className="absolute inset-x-6 top-5 flex items-center justify-between">
            ...
            <span className="rounded-full bg-white/12 px-3 py-1 text-xs font-bold text-white/80 backdrop-blur">Rəqəmsal profil</span>
          </div>
          <div className="absolute -bottom-16 left-6">
            {profile.avatar_url ? (
              <img src={profile.avatar_url} alt={profile.name} className="size-32 rounded-[2rem] border-4 border-white object-cover shadow-lift" />
            ) : (
        ...
        <div className="px-6 pb-7 pt-20">
          <p className="text-sm font-black uppercase tracking-[0.16em] text-cyan-700">{profile.profession}</p>
          <h1 className="mt-2 text-4xl font-black leading-tight tracking-tight">{profile.name}</h1>
          {profile.bio ? <p className="mt-4 leading-7 text-slate-600">{profile.bio}</p> : null}
          {profile.location ? (
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            {profile.instagram ? <Social href={profile.instagram} icon={<Instagram size={18} />} label="Instagram" /> : null}
            {profile.tiktok ? <Social href={profile.tiktok} icon={<Music2 size={18} />} label="TikTok" /> : null}
            {profile.website ? <Social href={profile.website} icon={<Globe size={18} />} label="Website" /> : null}
            <Social href={`/u/${profile.slug}/vcard`} icon={<UserPlus size={18} />} label="Kontaktı yadda saxla" />
          </div>
            ...
            <div className="mt-8">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                {profile.gallery.map((image) => (
                  <img key={image} src={image} alt={`${profile.name} portfolio`} className="aspect-square rounded-2xl object-cover" />
                ))}
              </div>
            </div>
          ) : null}
          ...
          <div className="mt-8 flex items-center gap-4 rounded-3xl border border-slate-200 bg-slate-50 p-4">
            <img src={qr} alt="Profile QR code" className="size-24 rounded-2xl bg-white p-2" />
            <div>
              <div className="flex items-center gap-2 text-sm font-black text-slate-950">
                <QrCode size={16} /> QR ehtiyat nüsxəsi
              </div>
              <p className="mt-2 text-sm leading-6 text-slate-600">NFC mövcud olmadıqda kamera ilə skan edin.</p>
            </div>
          </div>
        </div>
      </section>
