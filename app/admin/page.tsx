import { loginAdmin, logoutAdmin, saveProfile, toggleProfile } from "@/app/admin/actions";
      ...
      <div className="mx-auto max-w-7xl">
        <header className="rounded-[2rem] border border-white/80 bg-white/75 p-5 shadow-[0_22px_80px_rgba(15,23,42,0.08)] backdrop-blur-xl sm:p-6">
          <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
            <div>
              ...
              <h1 className="mt-4 text-4xl font-black tracking-tight text-slate-950 sm:text-5xl">Client profiles</h1>
              <p className="mt-3 max-w-2xl text-base leading-7 text-slate-600">
                Create, edit, activate, and share premium NFC profile pages from one focused dashboard.
              </p>
            </div>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            <Stat icon={<Users size={19} />} value={String(profiles.length)} label="Total profiles" />
            <Stat icon={<Activity size={19} />} value={String(enabledCount)} label="Active profiles" />
            <Stat icon={<Database size={19} />} value={hasSupabaseEnv() ? "Live" : "Demo"} label="Data mode" />
          </div>
        </header>
        ...
        {!hasSupabaseEnv() ? (
          <div className="mt-5 flex gap-3 rounded-3xl border border-amber-200 bg-amber-50 p-5 text-sm font-semibold leading-6 text-amber-900">
            ...
            Supabase environment values are missing. Demo profile is visible, but real create/edit needs `.env.local`.
          </div>
        ) : null}
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[0.86fr_1.14fr]">
          <div className="self-start rounded-[2rem] border border-slate-200 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.07)] sm:p-6">
            <div className="mb-6 flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-4 text-2xl font-black tracking-tight">Create profile</h2>
                <p className="mt-2 text-sm leading-6 text-slate-500">Add a new NFC profile with contact actions, social links, gallery, and vCard support.</p>
              </div>
            </div>
            <ProfileForm />
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="text-sm font-black uppercase tracking-[0.18em] text-cyan-700">Directory</p>
                <h2 className="mt-2 text-3xl font-black tracking-tight">Managed profiles</h2>
              </div>
            </div>
            ...
            {profiles.length === 0 ? (
              <div className="rounded-[2rem] border border-dashed border-slate-300 bg-white/70 p-8 text-center shadow-sm">
                ...
                <h3 className="mt-4 text-xl font-black tracking-tight">No profiles yet</h3>
                <p className="mt-2 text-sm leading-6 text-slate-500">Create your first profile from the form on the left.</p>
              </div>
            ) : null}
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-center gap-3">
        <div className="grid size-10 place-items-center rounded-2xl bg-slate-100 text-slate-700">{icon}</div>
        <div>
...
function ProfileCard({ profile, url, qr }: { profile: Profile; url: string; qr: string }) {
  return (
    <article className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.07)] transition duration-200 ease-out hover:-translate-y-0.5 hover:shadow-[0_24px_80px_rgba(15,23,42,0.1)]">
      <div className="flex flex-col justify-between gap-5 md:flex-row md:items-start">
        <div className="flex gap-4">
          {profile.avatar_url ? (
            <img src={profile.avatar_url} alt={profile.name} className="size-14 rounded-2xl object-cover" />
          ) : (
            <div className="grid size-14 place-items-center rounded-2xl bg-slate-950 text-xl font-black text-cyan-300">{profile.name.charAt(0)}</div>
          )}
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span
                className={`rounded-full px-3 py-1 text-xs font-black ${
                  profile.enabled ? "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200" : "bg-orange-50 text-orange-700 ring-1 ring-orange-200"
                }`}
              >
                {profile.enabled ? "Active" : "Disabled"}
              </span>
            </div>
            <p className="mt-1 text-sm font-bold text-slate-500">/{profile.slug}</p>
            {profile.profession ? <p className="mt-2 text-sm font-semibold text-slate-600">{profile.profession}</p> : null}
          </div>
        </div>
      ...
      <div className="mt-5 flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-600">
        ...
        <a href={url} className="min-w-0 break-all transition-colors hover:text-cyan-700">
          {url}
        </a>
      </div>
      ...
      <div className="mt-4 grid gap-3 rounded-3xl border border-slate-200 bg-white p-4 text-slate-950">
        ...
        <img src={qr} alt={`${profile.name} QR kodu`} className="h-28 w-28 rounded-2xl bg-slate-50 p-2 shadow-sm" />
      </div>
      ...
      <details className="group mt-4 rounded-3xl border border-slate-200 bg-white">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-5 py-4 text-sm font-black text-slate-950">
          <span className="inline-flex items-center gap-2">
            <WandSparkles size={17} className="text-cyan-700" /> Edit profile details
          </span>
          <ArrowUpRight className="transition-transform duration-200 ease-out group-open:rotate-45" size={17} />
        </summary>
...
function Login({ error }: { error: boolean }) {
  return (
    <main className="grid min-h-screen place-items-center bg-slate-950 px-4 py-10 text-white">
      ...
      <form action={loginAdmin} className="w-full max-w-md rounded-[2rem] border border-white/10 bg-white p-7 text-slate-950 shadow-[0_34px_110px_rgba(0,0,0,0.35)] sm:p-8">
        <div className="grid size-12 place-items-center rounded-2xl bg-slate-950 text-cyan-300">
          <LockKeyhole size={22} />
        </div>
        <p className="mt-5 text-sm font-black uppercase tracking-[0.18em] text-cyan-700">Admin login</p>
        <h1 className="mt-2 text-3xl font-black tracking-tight">Zia NFC panel</h1>
        <p className="mt-3 text-sm leading-6 text-slate-500">Sign in to manage client profiles and NFC card destinations.</p>
        {error ? (
        ...
        <label className="mt-6 block text-sm font-black text-slate-700">
          ...
          <div className="relative">
            <Mail className="pointer-events-none absolute left-4 top-1/2 mt-1 -translate-y-1/2 text-slate-400" size={17} />
            <input name="email" type="email" required className={`${inputClass} pl-11`} />
          </div>
        </label>
        <label className="mt-4 block text-sm font-black text-slate-700">
          Password
          <div className="relative">
            <LockKeyhole className="pointer-events-none absolute left-4 top-1/2 mt-1 -translate-y-1/2 text-slate-400" size={17} />
            <input name="password" type="password" required className={`${inputClass} pl-11`} />
          </div>
        </label>
...
function ProfileForm({ profile }: { profile?: Profile }) {
  return (
    <form action={saveProfile} encType="multipart/form-data" className="grid gap-4">
      <input type="hidden" name="id" value={profile?.id ?? ""} />
      <div className="grid gap-4 md:grid-cols-2">
        <Field name="name" label="Full name" defaultValue={profile?.name} required />
        <Field name="slug" label="Profile slug" defaultValue={profile?.slug} required />
        <Field name="profession" label="Profession" defaultValue={profile?.profession} />
        <Field name="phone" label="Phone" defaultValue={profile?.phone} />
        <Field name="whatsapp" label="WhatsApp" defaultValue={profile?.whatsapp} />
        <Field name="instagram" label="Instagram URL" defaultValue={profile?.instagram} />
        <Field name="tiktok" label="TikTok URL" defaultValue={profile?.tiktok} />
        <Field name="website" label="Website URL" defaultValue={profile?.website} />
      </div>
      ...
      <label className="block text-sm font-black text-slate-700">
        Portfolio URLs
        <textarea name="gallery" defaultValue={(profile?.gallery ?? []).join("\n")} rows={4} className={inputClass} />
      </label>
      ...
      <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm font-black text-slate-700">
        <input type="checkbox" name="enabled" defaultChecked={profile?.enabled ?? true} className="size-5 rounded accent-cyan-500" />
        Profile is active
      </label>
...
function UploadField({ name, label, multiple = false }: { name: string; label: string; multiple?: boolean }) {
  return (
    <label className="block rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm font-black text-slate-700 transition duration-200 ease-out hover:border-cyan-300 hover:bg-cyan-50/40">
      ...
      <input name={name} type="file" accept="image/*" multiple={multiple} className="mt-3 w-full text-sm font-semibold text-slate-500 file:mr-3 file:rounded-full file:border-0 file:bg-slate-950 file:px-4 file:py-2 file:text-sm file:font-black file:text-white" />
    </label>
  );
}
