  ...
  return (
    <section id="cards" className="bg-white px-4 py-20 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        >
          <p className="text-sm font-black uppercase tracking-[0.22em] text-sky-700">Kart kolleksiyası</p>
        ...
        <div className="mt-12 grid gap-10 md:grid-cols-3 md:gap-6 lg:gap-8">
          {cards.map((card, index) => (
            <div key={card.variant} className="flex flex-col items-center px-2 py-2">
              ...
              <p className="mt-3 text-xs font-bold uppercase tracking-[0.14em] text-sky-700">Üzərinə gəlin</p>
            </div>
          ))}
        </div>
      </div>
...
function FlipCard({ card }: { card: (typeof cards)[number] }) {
  return (
    <div className="nfc-card-flip w-full max-w-[320px] cursor-pointer outline-none focus-within:ring-4 focus-within:ring-sky-200/80 rounded-[1.35rem]" tabIndex={0}>
              ...
              <div className="grid place-items-center gap-3">
                <div
                  className={`grid size-14 place-items-center rounded-2xl ${
                    card.variant === "black" ? "bg-white/10 text-sky-300" : "bg-slate-950/8 text-slate-700"
                  }`}
                >
